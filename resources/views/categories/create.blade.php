<x-app-layout>
    <x-slot name="header">Create New Category</x-slot>
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                    <h2 class="text-base font-semibold text-slate-800 dark:text-slate-100">New Category</h2>
                    <p class="text-xs text-slate-400 mt-0.5">Add a new support category for tickets.</p>
                </div>
                ...
                <form action="{{ route('categories.store') }}" method="POST" class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                            ...
                            class="w-full text-sm rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 shadow-sm focus:border-blue-500 focus:ring-blue-500 resize-none @error('description') border-red-400 @enderror">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
